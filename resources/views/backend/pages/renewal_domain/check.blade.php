...
@section('fav_title', __('backend/renewal_domain.renewal_domain') )
...
<div class="app-title">
	...
	<ul class="app-breadcrumb breadcrumb">
		...
		<li class="breadcrumb-item active">{{ __('backend/renewal_domain.renewal_domain') }}</li>
	</ul>
</div>
	...
	<div class="col-md-12">
		<div class="card">
			...
			<div class="card-header">
				<div class="row">
					<div class="col-md-6"><h2><i class="{{ 'fa fa-table' }}"></i> {{ __('backend/renewal_domain.renewal_domain_list') }}</h2></div>
					<div class="col-md-6">
						<h2>
						...
						<a href="{{ route('admin.renewal_domain.add') }}" class="float-right btn btn-primary"><i class="fa fa-plus"></i> {{ __('backend/default.add_new') }}</a>
					</div>
			...
			<div class="card-body">
				...
				<div class="toggle-table-column alert-info br-2 p-2 mb-2 none">
					...
					<a href="#" class="toggle-vis" data-column="3"><b>{{ __('backend/renewal_domain.renewal_price') }}</small></b></a> |
				...
				<div class="table-responsive pt-1">
					<table id="datatable" class="table table-bordered table-hover display table-striped">
						...
						<thead class="alert-secondary">
							...
							<th>{{ __('backend/renewal_domain.renewal_price') }}</th>
						...
						<tbody>
							@foreach ($rows as $row)
							<tr class="{{ ($row->status == 0 || strpos($row->expiration_date->diffForHumans(), 'ago') !== false) ? 'deactive_':'warning_' }}">
								<td>{{ $loop->index+1 }}</td>
								<td>{{ $row->name }}</td>
								<td>{{ $row->mobile }}</td>
								<td class="text-center">{{ $row->renewal_price }}</td>
								<td>{{ $row->domain }}</td>
								<td>{{ $row->hosting }}</td>
								<td class="text-center px-0">
									<strong>{{ date('Y-m-d', strtotime($row->expiration_date)) }}</strong>
									<br>
									<small>{{ $row->expiration_date->diffForHumans() }}</small>
								</td>

								<td>{{ $row->status == 1 && !(strpos($row->expiration_date->diffForHumans(), 'ago') !== false) ? 'Active' : 'Deactive' }}</td>
								<td class="action">
									<a title="Renew" href="{{ asset('/admin/renewal_domain/add/'.$row->id) }}?name={{ urlencode($row->name) }}" class="btn btn-success"><i class="fa fa-retweet"></i></a>
								</td>
							</tr>
							@endforeach
						</tbody>
