				...
				<div class="table-responsive pt-1">
					<table id="datatable" class="table table-bordered table-hover display table-striped">
						...
						<thead class="alert-secondary">
							...
							<th class="text-center">{{ __('backend/renewal_domain.renewal_price') }}</th>
						...
						<tbody>
							...
							<tr class="{{ ($row->status == 0 || strpos($row->expiration_at->diffForHumans(), 'ago') !== false) ? 'deactive_':'' }}">
								...
								<td class="text-center">{{ $row->renewal_amount }}</td>
								...
								<td class="action">
									<div class="btn-group">
                                        ...
                                        @if($key == 0)
                                        <a title="Renewal History" href="{{ route($permission->route, $row->client_id) }}?name={{ urlencode($row->client->name) }}" class="btn btn-info"><i class="fa fa-history"></i></a>
                                        @elseif($key == 1)
                                        <a title="Renew" href="{{ asset('/admin/renewal_domain/add/'.$row->client_id) }}?name={{ urlencode($row->client->name) }}" class="btn btn-warning"><i class="fa fa-retweet"></i></a>
                                        @else
